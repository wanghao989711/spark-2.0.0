 ...
 */
// TaskSet是DAGScheduler创建的,提交给TaskScheduler
// TaskSchedulerImpl会为每个提交的TaskSet创建一个TaskSetManager
private[spark] class TaskSetManager(
  ...
  // 每个task有几个副本在运行，用于Speculate Task
  ...
  // key: taskId ,value: executorId -> failedTime
  ...
  // it is always null, it can only be leaf in the tree of Schedulales(with Pool being the node)
  var parent: Pool = null

  // is the current total size of the result of all the tasks that have finished.
  ...
  // 为executor分配Task，是从pendingTaskfor*中取出来的
  def resourceOffer(
      execId: String,
      host: String,
      ...
      dequeueTask(execId, host, allowedLocality) match {
        case Some((index, taskLocality, speculative)) =>
          ...
          if (serializedTask.limit > TaskSetManager.TASK_SIZE_TO_WARN_KB * 1024 &&
            ...
            // 如果序列化之后的task大小大于100KB会警告
            logWarning(s"Stage ${task.stageId} contains a task of very large size " +
              s"(${serializedTask.limit / 1024} KB). The maximum recommended task size is " +
              s"${TaskSetManager.TASK_SIZE_TO_WARN_KB} KB.")
          }

          // 将taskId添加到runningTasksSet 并且如果parent不为空，将parent的runningTask + 1
  ...
  // 被TaskSchedulerImpl调用
  override def executorLost(execId: String, host: String, reason: ExecutorLossReason) {
    // Re-enqueue any tasks that ran on the failed executor if this is a shuffle map stage,
    // and we are not using an external shuffle server which could serve the shuffle outputs.
    // The reason is the next stage wouldn't be able to fetch the data from this dead executor
    // so we would need to rerun these tasks on other executors.
    // 如果失败的是ShuffleMapTask 并且 external shuffle server没有开启
    if (tasks(0).isInstanceOf[ShuffleMapTask] && !env.blockManager.externalShuffleServiceEnabled) {
      for ((tid, info) <- taskInfos if info.executorId == execId) { // 属于失败的executor的task
        val index = taskInfos(tid).index // TaskInfo.index
        if (successful(index)) {// 如果该task已经成功了
          successful(index) = false // 将该task标记为没有成功
          copiesRunning(index) -= 1 // 将该task运行副本数-1
          tasksSuccessful -= 1 // 将成功task的计数-1
          addPendingTask(index) // 将该task加入pendingTasksForExecutor/pendingTasksForHost/pendingTasksForRack
          // Tell the DAGScheduler that this task was resubmitted so that it doesn't think our
          // stage finishes when a total of tasks.size tasks finish.
          // 告诉DAGScheduler重新提交了task
   ...
   */
  // 被TaskShcedulerImpl定期调用来检测是否有慢任务
  override def checkSpeculatableTasks(): Boolean = {
    ...
    // 最少有多少个task已经完成了，是个定值，用于定义慢任务
    val minFinishedForSpeculation = (SPECULATION_QUANTILE * numTasks).floor.toInt
    logDebug("Checking for speculative tasks: minFinished = " + minFinishedForSpeculation)

    if (tasksSuccessful >= minFinishedForSpeculation && tasksSuccessful > 0) {// 如果已经完成的任务个数大于等于阈值
      ...
      for ((tid, info) <- taskInfos) {// 遍历所有的taskInfos
        val index = info.index
        //没有成功的任务  && 运行副本为1 && 运行时间>阈值 && 还没有添加进慢任务列表
        if (!successful(index) && copiesRunning(index) == 1 && info.timeRunning(time) > threshold &&
          !speculatableTasks.contains(index)) {
          logInfo(
            "Marking task %d in stage %s (on %s) as speculatable because it ran more than %.0f ms"
              .format(index, taskSet.id, info.host, threshold))
          speculatableTasks += index // 添加进慢任务列表
          foundTasks = true // 标记找到慢任务
        }
      }
    }
    foundTasks
