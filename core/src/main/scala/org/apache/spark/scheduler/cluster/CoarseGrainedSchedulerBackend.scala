...
// This backend holds executors for the duration of the Spark job
private[spark]
class CoarseGrainedSchedulerBackend(scheduler: TaskSchedulerImpl, val rpcEnv: RpcEnv)
  // 可以向cluster manager请求资源
  extends ExecutorAllocationClient with SchedulerBackend with Logging
{
  ...
  class DriverEndpoint(override val rpcEnv: RpcEnv, sparkProperties: Seq[(String, String)])
    ...
    private def makeOffers() {
      ...
      // 所有可用的executors  (executorId, host, freeCores)
    ...
    private def launchTasks(tasks: Seq[Seq[TaskDescription]]) {
      for (task <- tasks.flatten) {
        ...
        if (serializedTask.limit >= maxRpcMessageSize) {
          ...
          // executorEndpointRef 对应的是 CoarseGrainedExecutorBackend
        }
      }
    }
  ...
  // 被StandaloneSchedulerBackend重写
  // 使用StandaloneAPPClient来向ClientEndpoint发送消息，ClientEndpoint收到消息然后向master发送消息
