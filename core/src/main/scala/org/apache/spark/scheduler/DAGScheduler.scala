 ...
 */
// application -> actions[jobs] -> stages -> taskSets -> tasks
...
class DAGScheduler(
  ...
  // SparkContext中创建DAGScheduler的时候调用的是该构造函数
  ...
  private[scheduler] def numTotalJobs: Int = nextJobId.get() // 一共提交了多少个job
  private val nextStageId = new AtomicInteger(0)

  // jobId 到 stageId的映射 因为一个job有多个stage ,所以是 Int -> HashSet[Int]
  private[scheduler] val jobIdToStageIds = new HashMap[Int, HashSet[Int]]
  // stageId 到 stage 的映射 Int -> Stage
  private[scheduler] val stageIdToStage = new HashMap[Int, Stage]
  // shuffleId 到 MapStage 的映射  shuffleId -> Mapside Stage
  ...
  private def getParentStagesAndId(rdd: RDD[_], firstJobId: Int): (List[Stage], Int) = {
    ...
    val id = nextStageId.getAndIncrement()//生成父stage的Id
    (parentStages, id)
  }
  ...
  def submitJob[T, U](
    ...
    partitions.find(p => p >= maxPartitions || p < 0).foreach { p => // 不合法的partition
  ...
  private def submitMissingTasks(stage: Stage, jobId: Int) {
    ...
    try {
      ...
      taskBinary = sc.broadcast(taskBinaryBytes) // 通过广播将已经序列化的task广播出去
    } catch {
    ...
    val tasks: Seq[Task[_]] = try { // 每个partition一个task
      stage match {
        case stage: ShuffleMapStage =>
          partitionsToCompute.map { id =>
    ...
    if (tasks.size > 0) {
      ...
      taskScheduler.submitTasks(new TaskSet(// 创建一个TaskSet
