 ...
 * internal data across the cluster. There exists one MemoryManager per JVM.
  *
  *  用于管理内存分配: 运行内存(Execution memory)和存储内存(Storage memory)
  *  运行内存指的是运行shuffle、join、sort、aggregation等计算时使用的内存
  *  存储内存指的是cache、propagating data所使用的内存
  *  每个JVM进程有一个MemoryManager
  *
  *  有两种实现: UnifiedMemoryManager(默认)和StaticMemoryManager
 */
private[spark] abstract class MemoryManager(
  ...
  // offHeapMemory为配置文件配置,Execution和Storage各占一半
   ...
   * Execution memory currently in use, in bytes.
    *
    * ExecutionMemory onHeap + offHeap
   */
   ...
   * Returns the execution memory consumption, in bytes, for the given task.
    *
    * Task的ExecutionMemory  onHeap + offHeap
   */
