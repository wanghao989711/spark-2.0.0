 ...
 * created.
 */
private[spark] case object TaskSchedulerIsSet // 告诉sc的TaskScheduler已经好了
...
private[spark] class HeartbeatReceiver(sc: SparkContext, clock: Clock)
  ...
  private[spark] var scheduler: TaskScheduler = null // sc的TaskScheduler
  ...
  // 单线程的ThreadPoolExecutor,用于请求让SparkContext杀掉executor
  ...
  override def receiveAndReply(context: RpcCallContext): PartialFunction[Any, Unit] = {
    ...
    // Messages sent and received locally 发送给自己
    ...
    case ExecutorRemoved(executorId) =>
      ...
      context.reply(true)
    case TaskSchedulerIsSet => // 是SparkContext创建之后发送给HaeartbeatReceiver的,用于通知TaskScheduler已经可用了
  ...
  private def expireDeadHosts(): Unit = {// 检查executor是否超时
    logTrace("Checking for hosts with no recent heartbeats in HeartbeatReceiver.")
    val now = clock.getTimeMillis()
    for ((executorId, lastSeenMs) <- executorLastSeen) {
      if (now - lastSeenMs > executorTimeoutMs) {// 如果超时
        logWarning(s"Removing executor $executorId with no recent heartbeats: " +
          s"${now - lastSeenMs} ms exceeds timeout $executorTimeoutMs ms")
        scheduler.executorLost(executorId, SlaveLost("Executor heartbeat " +
          s"timed out after ${now - lastSeenMs} ms")) //taskScheduler移除该executor
          // Asynchronously kill the executor to avoid blocking the current thread
        killExecutorThread.submit(new Runnable { // 异步让sc杀掉executor,为防止阻塞当前线程
          override def run(): Unit = Utils.tryLogNonFatalError {
            // Note: we want to get an executor back after expiring this one,
            // so do not simply call `sc.killExecutor` here (SPARK-8119)
            sc.killAndReplaceExecutor(executorId)// 让sc将该executor杀掉
          }
        })
        executorLastSeen.remove(executorId) // 在内存数据结构中将executor移除
      }
    }
  }
