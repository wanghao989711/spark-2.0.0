 ...
 *
  *一般来说,SecurityManager是应该在SparkEnv中实例化, 并且大部分想要访问SecurityManager的组件也应该在SparkEnv中访问。
  * 有一些比较特殊的例子,比如SparkEnv还没有被实例化,那么SecurityManager需要自己直接实例化。
  *
 ...
 *
  * Spark目前支持通过共享秘钥进行认证。
  * 认证可以通过配置参数spark.authenticate进行配置。
  * 这个参数控制了spark通信协议是否使用共享秘钥进行验证。这个验证以确保通信的两端有相同的共享秘钥才能进行通信。
  * 如果共享秘钥认证没有通过,则不能进行通信。
  *
 ...
 *
  *Spark UI也可以使用javax servlet过滤器来保证安全。用户可能想要保护UI上的一些数据不被其他的用户看见。
  * 可以通过spark.acls.enable spark.ui.view.acls spark.ui.view.acls.groups来进行控制用户查看权限。
  *
 ...
 * acls to work properly.
  *
  * spark.modify.acls spark.modify.acls.groups用来控制用户修改（kill application）权限。
 *
 ...
 * who always have permission to view or modify the Spark application.
  *
  * spark.admin.acls spark.admin.acls.groups 配置管理员用户，拥有查看和修改权限。
 *
...
private[spark] class SecurityManager(sparkConf: SparkConf)
  ...
  private val WILDCARD_ACL = "*" // 掩码

  private val authOn = sparkConf.getBoolean(SecurityManager.SPARK_AUTH_CONF, false) //默认为false
  // keep spark.ui.acls.enable for backwards compatibility with 1.0
  private var aclsOn =
    sparkConf.getBoolean("spark.acls.enable", sparkConf.getBoolean("spark.ui.acls.enable", false))

  // admin acls should be set before view or modify acls
  // 设置管理员acls和管理员用户组，必须放在最前面
  ...
  private val secretKey = generateSecretKey() // 生成秘钥
