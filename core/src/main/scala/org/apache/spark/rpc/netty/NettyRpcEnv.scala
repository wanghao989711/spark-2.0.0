...
private[netty] class NettyRpcEnv(
  ...
  private val dispatcher: Dispatcher = new Dispatcher(this) // 消息分发器
  ...
  def startServer(port: Int): Unit = {
    ...
    dispatcher.registerRpcEndpoint(
      // NettyRpcEnv启动服务的时候就已经注册了RpcEndpointVerifier,用于验证RpcEndpointRef是否在dispatcher中存在
  ...
  def asyncSetupEndpointRefByURI(uri: String): Future[RpcEndpointRef] = {
    ...
    val verifier = new NettyRpcEndpointRef(
      // RpcEndpointVerifier在NettyRpcEnv startServer的时候就已经注册给了dispatcher了
      // RpcEndpointAddress中的name为"endpoint-verifier"
      conf, RpcEndpointAddress(addr.rpcAddress, RpcEndpointVerifier.NAME), this)
    // 该消息最终会被dispatcher分发给RpcEndpointVerifier,RpcEndpointVerifier收到消息会调用自己的receiverAndReply方法
    // 最终会调用dispatcher的verify方法来验证endpointRef是否存在
