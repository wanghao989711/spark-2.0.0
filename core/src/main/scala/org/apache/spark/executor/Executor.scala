...
private[spark] class Executor(
  ...
  def launchTask(
    ...
    threadPool.execute(tr) // 调用的是Thread的start() 方法，实质是Runnable接口的run()
  }
  ...
  class TaskRunner(
    ...
    override def run(): Unit = {
      ...
      try {
        ...
        // 使用CoarseGrainedExecutorBackend的statusUpdate方法 用DriverEndpointRef向DriverEndpoint返回数据
