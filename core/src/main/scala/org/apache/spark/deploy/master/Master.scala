...
private[deploy] class Master(
    override val rpcEnv: RpcEnv,// 复写的trait RpcEndpoint中的rpcEnv
    address: RpcAddress,
    webUiPort: Int,
  ...
  override def onStart(): Unit = {
    ...
    checkForWorkerTimeOutTask = forwardMessageThread.scheduleAtFixedRate(new Runnable {
      override def run(): Unit = Utils.tryLogNonFatalError {
        // self: rpcEnv.endpointRef(this),就是master自己
        // 使用一个单独的线程专门用于定期向master发送CheckForWorkerTimeOut消息
    ...
    // 向masterMetricsSystem注册Source
    masterMetricsSystem.registerSource(masterSource)
    // 启动masterMetricsSystem
    masterMetricsSystem.start()
    // 启动applicationMetricsSystem
