 ...
 * - [[Unevaluable]]: an expression that is not supposed to be evaluated. 不可运算的
 ...
 *                       the same output data type. 两个孩子节点的输出类型要一样
 *
 */
abstract class Expression extends TreeNode[Expression] {
   ...
   */
  def deterministic: Boolean = children.forall(_.deterministic) // 确定的，稳定的
