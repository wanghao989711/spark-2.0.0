...
class AstBuilder extends SqlBaseBaseVisitor[AnyRef] with Logging {
  ...
  override def visitChildren(node: RuleNode): AnyRef = {
    if (node.getChildCount == 1) {
      node.getChild(0).accept(this)
        // RuleContext.accept(this[AstBuilder/visitor])
        // visitor.visitChildren(ruleContext)
        // 用visitor visit node的child
    } else {
      null
    }
  }
  ...
  override def visitSingleStatement(ctx: SingleStatementContext): LogicalPlan = withOrigin(ctx) {
    visit(ctx.statement).asInstanceOf[LogicalPlan]// 因为T是AnyRef，所以需要强转
  }
  // 表达式
  ...
  override def visitSingleTableIdentifier(
    ...
    // 创建了TableIdentifier case class
    visitTableIdentifier(ctx.tableIdentifier/*TableIdentifierContext*/)
  }
   ...
   */
  protected def expression(ctx: ParserRuleContext): Expression = typedVisit(ctx) // 已经进行了强转
  ...
  override def visitNamedExpression(ctx: NamedExpressionContext): Expression = withOrigin(ctx) {
    val e: Expression = expression(ctx.expression) // 进行类型转换
    if (ctx.identifier != null) {// 定义了别名
