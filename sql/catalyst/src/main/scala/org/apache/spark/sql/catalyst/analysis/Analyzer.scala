 ...
 * [[SessionCatalog]] and a [[FunctionRegistry]].
  *
  *
  * 继承自RuleExecutor， RuleExecutor中有execute方法，将batches中的所有batch中的所有rules作用于plan
 */
class Analyzer(   // 用于指定parser生成的parserTree中的变量的类型，以及表、列是否存在等
  ...
  def resolver: Resolver = { // 为一个函数 (String, String) => boolean
   ...
   */
  val extendedResolutionRules: Seq[Rule[LogicalPlan]] = Nil // 用于给用户扩展
   ...
   */
  object CTESubstitution extends Rule[LogicalPlan] { // Common Table Expression
