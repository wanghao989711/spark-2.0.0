...
object SQLConf {
  ...
  private val sqlConfEntries = java.util.Collections.synchronizedMap( // 同步的HashMap (ConfigEntry.key -> ConfigEntry)
    new java.util.HashMap[String, ConfigEntry[_]]())

  private def register(entry: ConfigEntry[_]): Unit = sqlConfEntries.synchronized {// 注册配置信息
  ...
  /*
  * 调用SQLConfigBuilder的apply方法,先new ConfigBuilder,
  * 然后通过onCreate方法为_onCreate赋值Option[register(ConfigEntry[_] => Unit)],
  * 这样_onCreate就绑定了一个回调函数。
  * 调用stringConf生成了一个TypedConfigBuilder类,并以ConfigBuilder(this)作为参数parent,
  * 调用TypedConfigBuilder类的createWithDefault方法时会调用parent的_onCreate方法,将ConfigEntry注册到sqlConfEntries HashMap中
  * */
