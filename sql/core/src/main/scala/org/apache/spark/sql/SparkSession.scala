...
class SparkSession private(
   ...
   * @since 2.0.0
   */
  def udf: UDFRegistration = sessionState.udf // udf注册器，注册到FunctionRegistry的HashMap中了
   ...
   */
  @transient
  lazy val emptyDataFrame: DataFrame = { // 创建空的DataFrame
   ...
   */
  @Experimental
  def emptyDataset[T: Encoder]: Dataset[T] = { // 创建空的Dataset,类型为T
   ...
   */
  @Experimental
  def createDataset[T : Encoder](data: Seq[T]): Dataset[T] = { // 从Scala对象集合创建DataSet
    val enc = encoderFor[T]
    val attributes = enc.schema.toAttributes
    val encoded = data.map(d => enc.toRow(d).copy())
    val plan = new LocalRelation(attributes, encoded) // LogicalPlan
    Dataset[T](self, plan)
  }
   ...
   */
  @Experimental
  def createDataset[T : Encoder](data: RDD[T]): Dataset[T] = {// 从RDD创建DataSet
    val enc = encoderFor[T]
    val attributes = enc.schema.toAttributes
    val encoded = data.map(d => enc.toRow(d))
    val plan = LogicalRDD(attributes, encoded)(self) // LogicalPlan
    Dataset[T](self, plan)
  }
   ...
   */
  @Experimental
  def range(start: Long, end: Long, step: Long, numPartitions: Int): Dataset[java.lang.Long] = {// 从用Range来生成DataSet
   ...
   * @since 2.0.0
   */
  def read: DataFrameReader = new DataFrameReader(self) // 里面有一些从数据源创建DataFrame的方法
   ...
   */
  @Experimental
  def readStream: DataStreamReader = new DataStreamReader(self) // 从流式数据创建DataFrame
...
object SparkSession {
  ...
  class Builder extends Logging {
    ...
    def getOrCreate(): SparkSession = synchronized {
      ...
      var session = activeThreadSession.get() // ThreadLocal
      ...
      SparkSession.synchronized {
        ...
        session = defaultSession.get() // global
        if ((session ne null) && !session.sparkContext.isStopped) {
          options.foreach { case (k, v) => session.conf.set(k, v) }
          if (options.nonEmpty) {
            logWarning("Use an existing SparkSession, some configuration may not take effect.")
